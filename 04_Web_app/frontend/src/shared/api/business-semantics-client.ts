...
const VALIDATION_KEYS = ["contract_name", "schema_version", "validation_id", "status", "job_creation_allowed", "file_validation", "model_limitations", "geo_points"] as const;
...
const CATALOG_KEYS = ["contract_name", "schema_version", "catalog_version", "status", "display_text", "geographies_n", "entries"] as const;
const WORKSPACE_KEYS = ["contract_name", "schema_version", "catalog_version", "status", "display_text", "total_budget_rub", "campaigns_n", "geographies_n", "rows"] as const;
...
function geo(value: unknown, regions = false): boolean {
  ...
  return coordinates(value) && (value.region_id === null || text(value.region_id)) && (value.region_display_name === null || text(value.region_display_name));
}
...
export function parseValidationViewV2(value: unknown, expectedValidationId: string): ValidationResultV2 {
  ...
  if (!moneyNear(geoBudget, fileValidation.requested_budget_rub as number)) throw new UnsupportedBusinessSemanticsContractError();
...
function validationGeo(value: unknown, requested: number): boolean {
  if (!record(value) || !exact(value, ["geo_id", "geo_display_name", "latitude", "longitude", "coordinates_status", "budget_rub", "budget_share", "channels", "has_model_limitations"]) || !geo({ geo_id: value.geo_id, geo_display_name: value.geo_display_name, latitude: value.latitude, longitude: value.longitude, coordinates_status: value.coordinates_status, region_id: null, region_display_name: null }, true) || !nonNegative(value.budget_rub) || !Array.isArray(value.channels) || !value.channels.every(channel) || !uniqueBy(value.channels, (item) => (item as JsonRecord).channel_id as string) || typeof value.has_model_limitations !== "boolean") return false;
