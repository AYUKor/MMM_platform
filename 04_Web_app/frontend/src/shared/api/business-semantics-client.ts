...
const FEDERAL_KEYS = ["status", "title", "description", "policy_version", "package_id", "source_rows_count", "source_budget_rub", "allocated_budget_rub", "difference_rub", "geo_count", "method_display_name", "channels", "business_directions", "mixed_local_overlap", "information", "warnings", "errors", "breakdown"] as const;
const FEDERAL_BREAKDOWN_KEYS = ["business_direction", "channels", "source_rows_count", "source_budget_rub", "allocated_budget_rub", "difference_rub", "geo_count"] as const;
...
function federalAllocation(value: unknown): boolean {
  ...
  return businessDirections.length === 1 ? positiveIntegerOrNull(value.geo_count) && value.geo_count !== null : value.geo_count === null;
}
