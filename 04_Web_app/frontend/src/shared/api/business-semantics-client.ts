...
const FEDERAL_TOLERANCE_RUB = 0.01;
...
const VALIDATION_KEYS = ["contract_name", "schema_version", "validation_id", "status", "job_creation_allowed", "file_validation", "federal_allocation", "model_limitations", "map_coverage", "geo_points"] as const;
const FEDERAL_KEYS = ["status", "title", "description", "policy_version", "package_id", "source_rows_count", "source_budget_rub", "allocated_budget_rub", "difference_rub", "geo_count", "declared_geo_count", "ready_geo_count", "excluded_geo_count", "denominator_policy_version", "lmax", "required_period_start", "required_period_end", "method_display_name", "channels", "business_directions", "mixed_local_overlap", "information", "warnings", "errors", "breakdown"] as const;
const FEDERAL_BREAKDOWN_KEYS = ["business_direction", "channels", "source_rows_count", "source_budget_rub", "allocated_budget_rub", "difference_rub", "geo_count", "declared_geo_count", "ready_geo_count", "excluded_geo_count", "period_start", "period_end"] as const;
...
export function parseValidationViewV2(value: unknown, expectedValidationId: string): ValidationResultV2 {
  ...
  if (!federalAllocation(value.federal_allocation)) throw new UnsupportedBusinessSemanticsContractError();
...
function statusMessages(value: unknown): value is JsonRecord[] { return Array.isArray(value) && value.every((item) => record(item) && exact(item, ["code", "display_text"]) && text(item.code) && text(item.display_text)) && uniqueBy(value, (item) => item.code as string); }
function federalAllocation(value: unknown): boolean {
  const countOrNull = (item: unknown) => item === null || nonNegativeInteger(item);
  const dateOrNull = (item: unknown) => item === null || isoDate(item);
  if (!record(value) || !exact(value, FEDERAL_KEYS) || !enumValue(value.status, ["none", "available", "error"] as const) || !nonNegativeInteger(value.source_rows_count) || !nonNegative(value.source_budget_rub) || !nonNegative(value.allocated_budget_rub) || !nonNegative(value.difference_rub) || !countOrNull(value.declared_geo_count) || !countOrNull(value.ready_geo_count) || !countOrNull(value.excluded_geo_count) || !countOrNull(value.lmax) || !dateOrNull(value.required_period_start) || !dateOrNull(value.required_period_end) || typeof value.mixed_local_overlap !== "boolean" || !Array.isArray(value.channels) || !value.channels.every(channel) || !uniqueBy(value.channels, (item) => (item as JsonRecord).channel_id as string) || !uniqueStrings(value.business_directions) || !statusMessages(value.information) || !statusMessages(value.warnings) || !statusMessages(value.errors) || !Array.isArray(value.breakdown)) return false;
  if (!value.breakdown.every((item) => record(item) && exact(item, FEDERAL_BREAKDOWN_KEYS) && text(item.business_direction) && Array.isArray(item.channels) && item.channels.every(channel) && uniqueBy(item.channels, (channelItem) => (channelItem as JsonRecord).channel_id as string) && nonNegativeInteger(item.source_rows_count) && nonNegative(item.source_budget_rub) && nonNegative(item.allocated_budget_rub) && nonNegative(item.difference_rub) && Math.abs((item.difference_rub as number) - Math.abs((item.source_budget_rub as number) - (item.allocated_budget_rub as number))) <= 1e-6 && positiveIntegerOrNull(item.geo_count) && countOrNull(item.declared_geo_count) && countOrNull(item.ready_geo_count) && countOrNull(item.excluded_geo_count) && dateOrNull(item.period_start) && dateOrNull(item.period_end))) return false;
  if (value.status !== "available") return value.title === null && value.description === null && value.policy_version === null && value.package_id === null && value.source_rows_count === 0 && value.source_budget_rub === 0 && value.allocated_budget_rub === 0 && value.difference_rub === 0 && value.geo_count === null && value.declared_geo_count === null && value.ready_geo_count === null && value.excluded_geo_count === null && value.denominator_policy_version === null && value.lmax === null && value.required_period_start === null && value.required_period_end === null && value.method_display_name === null && value.channels.length === 0 && value.business_directions.length === 0 && value.mixed_local_overlap === false && value.information.length === 0 && value.warnings.length === 0 && value.breakdown.length === 0 && (value.status === "none" ? value.errors.length === 0 : value.errors.length > 0);
  if (!text(value.title) || !text(value.description) || !text(value.policy_version) || !text(value.package_id) || (value.denominator_policy_version !== null && !text(value.denominator_policy_version)) || !text(value.method_display_name) || value.source_rows_count < 1 || value.difference_rub > FEDERAL_TOLERANCE_RUB || Math.abs(value.difference_rub - Math.abs(value.source_budget_rub - value.allocated_budget_rub)) > 1e-6 || value.errors.length > 0 || value.breakdown.length === 0 || ((value.required_period_start === null) !== (value.required_period_end === null)) || (value.required_period_start !== null && value.required_period_end !== null && value.required_period_start > value.required_period_end)) return false;
  const breakdown = value.breakdown as JsonRecord[];
  const businessDirections = value.business_directions as string[];
  const directions = [...new Set(breakdown.map((item) => item.business_direction as string))].sort();
  const totalRows = breakdown.reduce((sum, item) => sum + (item.source_rows_count as number), 0);
  const sourceBudget = breakdown.reduce((sum, item) => sum + (item.source_budget_rub as number), 0);
  const allocatedBudget = breakdown.reduce((sum, item) => sum + (item.allocated_budget_rub as number), 0);
  if (directions.length !== businessDirections.length || directions.some((direction, index) => direction !== businessDirections[index]) || totalRows !== value.source_rows_count || Math.abs(sourceBudget - value.source_budget_rub) > 1e-6 || Math.abs(allocatedBudget - value.allocated_budget_rub) > 1e-6) return false;
  if (value.declared_geo_count !== null && value.ready_geo_count !== null && value.excluded_geo_count !== null && value.declared_geo_count !== value.ready_geo_count + value.excluded_geo_count) return false;
  return value.geo_count === null || (positiveIntegerOrNull(value.geo_count) && value.geo_count === value.ready_geo_count);
}
