...
Если кампаний не найдено, `recommended_action` предлагает проверить обязательную
колонку `campaign_name`. Если найдено несколько кампаний, действие предлагает
разделить их на отдельные файлы. Это разные рекомендации при одном стабильном
machine code.
...
Все `checks[].display_text` сформулированы по-русски. Machine codes остаются
отдельными и не предназначены для показа вместо пользовательского текста.

## Actionable ValidationIssue

`blocking_errors[]` и `warnings[]` сохраняют существующие обязательные поля:
...
Для нового интерфейса добавлен optional набор из трех полей:

- `what` — что обнаружено;
- `why` — почему это важно для расчета или рекомендации;
- `recommended_action` — что пользователь может сделать дальше.

Если присутствует одно из этих полей, контракт требует все три. Frontend может
показать их отдельными блоками. `display_text` остается коротким совместимым
fallback для старых клиентов и не содержит внутреннюю инженерную терминологию.

Guidance заполняется для следующих стабильных codes:

- `CAMPAIGN_COUNT_NOT_ONE` — отдельно для нуля и нескольких кампаний;
- `UNSUPPORTED_MODEL_CELLS`;
- `MODEL_CAUTION_CELLS`;
- `MODEL_DIAGNOSTIC_CELLS`;
- `CAMPAIGN_VALIDATION_FAILED`.

Причины не додумываются. Например, caution сообщает только о зафиксированном
ограничении качества и не утверждает конкретную причину, которой нет в
исходных gate-данных.
...
названиям. До появления фиксированной географической проекции и утвержденного
контура России frontend не визуализирует даже переданный массив `geo_points`
как карту и сохраняет controlled unavailable state.
  ...
  "warnings": [
    {
      ...
      "severity": "warning",
      "display_text": "Строк, результаты которых требуют осторожной интерпретации: 1.",
      "what": "Строк с ограничениями по использованию оценки: 1.",
      "why": "Для указанного сочетания оценка разрешена только с ограничениями качества.",
      "recommended_action": "Проверьте отмеченную строку и не используйте ее как единственное основание для перераспределения бюджета.",
      "scope": "model",
      "recoverable": true,
      "source_row_ids": [],
      "affected_cells": []
...
- `ValidationIssue.what`, `why` и `recommended_action` не входят в required
  fields, но при использовании возвращаются полным набором;
- старые validation fixtures без preview проходят parser/schema validation и
  round-trip без добавления `preview: null` или guidance-полей со значением
  `null`;
- существующие upload, validation и job URLs не изменены;
- OpenAPI document version поднята до `1.2.1` как additive API extension.
