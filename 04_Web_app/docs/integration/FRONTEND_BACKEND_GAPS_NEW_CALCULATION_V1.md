...
| точки для карты | optional `preview.geo_points[]` |
...
|---|---|---|---|
| Координаты географий | optional `ValidationResult.preview.geo_points[]` с `latitude`, `longitude` и budget | массив в текущем application runtime отсутствует | карта показывает «данные пока недоступны»; frontend не геокодирует названия и не хардкодит координаты |
