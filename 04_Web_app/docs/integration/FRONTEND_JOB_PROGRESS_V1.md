...
`safe_candidates` и `blocked_candidates` не превращаются из `null` в zero;
zero не используется как сообщение об отсутствии безопасных вариантов.
...
- Vitest: `183/183` passed;
- production Vite build: passed;
- full Playwright regression on system Chrome: `77/77` passed;
