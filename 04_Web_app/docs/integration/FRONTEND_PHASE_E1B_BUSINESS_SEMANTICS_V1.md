...
| Только артефакты отчета | `GET /api/v1/jobs/{job_id}/result-view` | узкая projection поля `report` из `job_result_view_v1@1.0.0` |
...
запрещен. V1 result-view используется только отдельным artifact-only parser:
он сверяет `job_id`/`result_id` с v2 и возвращает status, безопасные download
metadata, sheets и working media-plan artifact. Campaign, KPI, ROAS, budgets,
scenarios, recommendation и reliability из v1 в его типе отсутствуют.
Lifecycle endpoints загрузки и создания job остаются orchestration boundary,
но пользовательское содержание проверки берется только из
...
## Report artifacts boundary

`job_result_view_v2` не публикует report/artifact metadata или download path и
по-прежнему единолично определяет все бизнес-значения результата. Для
восстановления скачивания отдельный узкий client читает только `report` subtree
из совместимого `job_result_view_v1`, проверяет identity результата, opaque
artifact ID, XLSX metadata и канонический path
`/api/v1/artifacts/{artifact_id}/download`, затем создает новую artifact-only
projection. V1 semantic fields не попадают в возвращаемый тип.

Ссылка показывается только при permission `report.download`; backend повторно
проверяет permission и integrity файла. Итоговый marketer Excel поддержан.
Отдельный `working_media_plan_xlsx` отображается и скачивается при реальном
`status=ready`, но текущий backend его не производит и честно публикует
`unavailable`; CSV не используется как подмена XLSX.
...
| Final Excel report | ready / 16 416 bytes / 3 sheets / XLSX `PK` signature |
...
| Fixture/full Playwright regression | passed: 157 tests; 1 unrelated opt-in live suite skipped |
| Chromium automated acceptance | passed |
| Live backend acceptance without interception | passed: 1 test; real final XLSX downloaded and verified |
| Safari manual smoke | passed: login, Home, Result, Validation и Model |
| Light/dark/mobile visual review | passed: 26 PNG |
...
2. V2 result не содержит artifact metadata/download path; transport отчета
   изолирован в artifact-only v1 projection без legacy business semantics.
   Отдельный working media-plan XLSX текущим backend не производится.
