...
fixtures. Review fixture намеренно содержит `geo_points`: E2E подтверждает,
что координаты не превращаются в min/max-normalized scatter и карта остается
в controlled unavailable state.
...
- карта остается в контролируемом состоянии недоступности даже при наличии
  optional `preview.geo_points`, пока не утверждены фиксированная проекция и
  контур России;
...
текущий application runtime не публикует координаты, а продуктовый контур не
фиксирует географическую проекцию и контур России. Поэтому карта остается в
состоянии «данные пока недоступны» даже для ответа с `geo_points`. Frontend не
растягивает точки относительно min/max текущей кампании, не выводит координаты
из названий, не обращается к внешнему geocoder и не считает наличие
budget-by-geo доказательством готовности карты.
...
- Карта недоступна до появления утвержденной фиксированной проекции и контура
  России; одного массива `geo_points` недостаточно. Budget-by-geo chart
...
6. Независимо от наличия `geo_points` карта остается в controlled unavailable
   state до появления фиксированной географической проекции и контура России.
...
- карта явно сообщает о недоступности данных даже при наличии `geo_points` и
  не изображает min/max-normalized scatter как географическую карту;
