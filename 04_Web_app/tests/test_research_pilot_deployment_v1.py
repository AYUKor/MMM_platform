...
class ResearchPilotDeploymentTest(unittest.TestCase):
    ...
    def test_render_is_loopback_panel_free_and_secret_free(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            backend_service = (output / "x5-mmm-backend.service").read_text(encoding="utf-8")
            ...
            self.assertTrue(
                config["paths"]["optimizer_policy_path"].endswith("optimizer_decision_policy_v3.yaml")
            )
            self.assertIn("EnvironmentFile=-/etc/x5-mmm/backend.env", backend_service)
